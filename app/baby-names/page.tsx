...
function Ranking({ id, title, names }: { id:string; title: string; names: typeof popularBoys }) {
  return <section className="ranking-section" id={id}><div className="ranking-heading"><h2>{title}</h2><span>2025 U.S. rank</span></div><ol className="ranking-list">{names.map((item) => <li key={item.slug}><Link href={`/baby-names/${item.slug}`}><span>{item.rank}</span><strong>{item.name}</strong><small>{item.meaning}</small><b>Explore →</b></Link></li>)}</ol></section>;
}
