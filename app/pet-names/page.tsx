...
export default function PetNamesPage() {
  return <InfoLayout eyebrow="The Namekind Top 100" title="100 pet names with real personality" intro="Browse familiar favorites, nature names, food names, mythic choices, and warm human-style names—then let our pet questionnaire find the right fit for your companion.">
    ...
    <section className="category-index"><div className="ranking-heading"><h2>Explore by personality</h2><span>Curated guides</span></div><div className="category-card-grid">{petCategories.map(item=><Link key={item.slug} href={`/pet-names/categories/${item.slug}`}><span>{item.eyebrow}</span><strong>{item.title}</strong><small>{item.description}</small><b>Explore guide →</b></Link>)}</div></section>
